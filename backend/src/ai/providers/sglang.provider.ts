...
/**
 * SGLang Provider（预留实现）。
 */
export class SGLangProvider extends BaseProvider {
  readonly code = 'sglang';

  constructor() {
    super(SGLangProvider.name);
  }
  ...
  async generate(): Promise<string> {
    throw new Error('SGLang provider not implemented in Stage 4');
  }
