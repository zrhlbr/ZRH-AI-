/**
 * Mobile Chat UI V5.0 本地验证截图：PC / Pad / Phone，目标 https://ai.zrhtech.com（测试栈）
 * 校验：空会话无品牌区、无底部导航、输入框 fixed 贴底、Header 56px、无横向滚动、消息右左布局。
 * 输出：docs/ux-v5-screenshots/public-v5-*.png
 */
...
async function main() {
  ...
  const username = `ux5pub_${suffix}`;
  const password = `Ux5Pub!${suffix}`;
  const reg = await api('/auth/register', {
    method: 'POST',
    body: JSON.stringify({ username, password, displayName: 'UX5 公网验证', acceptTerms: true, acceptPrivacy: true }),
  });
  ...
  for (const vp of VIEWPORTS) {
    ...
    await page.screenshot({ path: path.join(OUT, `public-v5-${vp.name}-01-login.png`) });
    ...
    await page.screenshot({ path: path.join(OUT, `public-v5-${vp.name}-02-home.png`) });
    ...
    await page.screenshot({ path: path.join(OUT, `public-v5-${vp.name}-03-chat-empty.png`) });
    ...
    await page.screenshot({ path: path.join(OUT, `public-v5-${vp.name}-04-chat-sent.png`) });

    // 输入框聚焦（移动端模拟键盘弹出前的聚焦态）
    await input.click();
    await page.waitForTimeout(600);
    await page.screenshot({ path: path.join(OUT, `public-v5-${vp.name}-05-chat-focus.png`) });

    // 抽屉菜单（手机/平板）
    if (vp.drawer) {
      await page.locator('header button').first().click();
      await page.waitForTimeout(900);
      await page.screenshot({ path: path.join(OUT, `public-v5-${vp.name}-06-drawer.png`) });
